...
# Update the extract_keywords_from_thought function to handle dictionary responses
def extract_keywords_from_thought(response):
    # Check if the response is a dictionary with a 'text' key
    if isinstance(response, dict) and 'text' in response:
        text = response['text']
    else:
        # If the response is not in the expected format, return an empty string
        return ""
    
    try:
        # Assuming keywords are listed after 'Keywords:' on a new line
        keywords_line = text.split("\nKeywords:")[1].strip()
        # Extract the actual keywords (assuming they are comma-separated)
        keywords = keywords_line.split(',')
        # Join the keywords back into a single string
        keywords_str = ' '.join(keywords).strip()
        return keywords_str
    except IndexError:
        # If the keyword line isn't found, return an empty string
        return ""

# Update the generate_thought function to pass only the 'text' to extract_keywords_from_thought
def generate_thought(question):
    ...
    # Ensure we pass only the 'text' part of the response to extract_keywords_from_thought
    if isinstance(response, dict) and 'text' in response:
        text = response['text']
        keywords = extract_keywords_from_thought(text)
    else:
        keywords = ""
